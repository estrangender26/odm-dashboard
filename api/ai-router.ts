...
const MODULE_DATA_TERMS =
  /\b(active module|dashboard data|module data|this dashboard|these records|this module|work orders?|tasks?|task count|kpis?|equipment|smp|manuals?|documents?|folders?|files?|schedule|critical path|milestones?|governance|inspection|ppp|responsible|ownership|coverage|uploads?|records?|my dashboard|our dashboard)\b/i;
...
const CURRENT_PPP_ONLY_PATTERN =
  /\bcurrent\s+ppp|current\s+doer|current\s+execution\s+doer\b/i;
