...
describe("Supabase Storage upload rollback flags", () => {
  it("defaults every module to the legacy path", () => {
    const value = flags();
    expect(value).toEqual({ global: false, om: false, governance: false, smp: false, projects_without_ppp: false });
    ...
    expect(isStorageUploadEnabled("projects_without_ppp", value)).toBe(false);
  });
  ...
  it.each([
    ...
    for (const other of ["om", "governance", "smp", "projects_without_ppp"] as const) {
