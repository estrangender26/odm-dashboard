...
describe("primaveraLite baseline router", () => {
  ...
  it("capture bumps revision exactly once", async () => {
    ...
    expect(captured.revision).toBe(loaded.revision + 1);
  ...
  it("pre-existing schedule staleness remains stale after capture", async () => {
  ...
  it("second baseline does not mutate first baseline", async () => {
    ...
    await caller.primaveraLite.updateActivity({
      ...
      expectedRevision: loaded4.revision,
      activityId: (await testDb.select({ id: ganttActivities.id }).from(ganttActivities).where(eq(ganttActivities.projectId, created.project.id)))[0].id,
      changes: { plannedStart: "2026-10-01", plannedFinish: "2026-10-05", originalDurationDays: 5 },
    });
  ...
  it("variance calculations are correct for start and finish", async () => {
    ...
    const activity = await caller.primaveraLite.createActivity({
      ...
      access: editorToken,
      expectedRevision: loaded.revision,
      activity: { activityName: "Task A", plannedStart: "2026-09-10", plannedFinish: "2026-09-10", originalDurationDays: 1 },
    });
    ...
    // positive variance: current later than baseline
    let currentRevision = captured.revision;
    await caller.primaveraLite.updateActivity({
      slug: created.project.slug,
      access: editorToken,
      expectedRevision: currentRevision,
      activityId: activity.activity.id,
      changes: { plannedStart: "2026-09-15", plannedFinish: "2026-09-15", originalDurationDays: 1 },
    });
    currentRevision += 1;
    await caller.primaveraLite.runSchedule({ slug: created.project.slug, access: adminToken, expectedRevision: currentRevision });
    currentRevision += 1;
    ...
    // negative variance: current earlier than baseline
    await caller.primaveraLite.updateActivity({
      slug: created.project.slug,
      access: editorToken,
      expectedRevision: currentRevision,
      activityId: activity.activity.id,
      changes: { plannedStart: "2026-09-08", plannedFinish: "2026-09-08", originalDurationDays: 1 },
    });
    currentRevision += 1;
    await caller.primaveraLite.runSchedule({ slug: created.project.slug, access: adminToken, expectedRevision: currentRevision });
  ...
  it("null-date variance behavior", async () => {
    ...
    expect(compare.comparisons[0].baselineScheduledStart).toBeNull();
