...
(function (global) {
  ...
  function buildAiInsightPayload(insight, rows) {
    ...
    if (type === 'critical-issues-immediate-action') {
      ...
      payload.criteria = 'Pareto top 20% equipment assets to raise in S/4, ranked by negative finding count.';
      ...
      payload.summary.push(contributors.length + ' S/4 equipment candidates');
      if (!payload.rows.length) payload.notes = 'No S/4 equipment candidates found for the current filters. Asset Tag / Equipment ID may be missing.';
      return payload;
    }
  ...
  function buildReconciledInsight(insight, rows) {
    ...
    if (type === 'critical-issues-immediate-action') {
      enriched.description = counts.entityCount + ' Pareto top-20% equipment asset' + (counts.entityCount === 1 ? '' : 's') + ' should be raised to S/4, accounting for ' + counts.recordCount + ' negative finding' + (counts.recordCount === 1 ? '' : 's') + '.';
      enriched.metric = counts.entityCount + ' S/4 equipment • ' + counts.recordCount + ' findings';
  ...
  function getSummaryCountLabel(kind, groups, rows, payload, counts) {
    ...
    if (kind === 'critical-pareto') return resolved.entityCount + ' S/4 equipment candidates • ' + resolved.recordCount + ' findings';
  ...
  function getSummaryViewLabel(kind, groups) {
    if (kind === 'critical-pareto') return groups.length + ' equipment assets to raise to S/4, sorted by finding count descending.';
