...
export function generateExecutiveContent(
  ...
  if (recovery.length > 0) {
    ...
    const milestoneCount = recovery.map(f => f.milestones.filter(m => m.status === 'gap').length).reduce((a, b) => Math.max(a, b), 0);
    headline = milestoneCount > 1 ? 
      names + ' must close ' + milestoneCount + ' Pre-PPP milestones' : 
      names + ' must close Pre-PPP milestones';
  ...
  if (recovery.length > 0) {
    ...
    const names = gateReady.map(f => f.shortName.split(' ')[0]).join(' and ');
    const datesArr = gateReady.map(f => new Date(f.pppStartDate).toLocaleDateString('en-US', { month: 'short', day: 'numeric' }));
    const uniqueDates = datesArr.filter((v, i, a) => a.indexOf(v) === i);
    const dates = uniqueDates.join(' and ');
  ...
  if (gateReady.length > 0) {
    ...
    const names = recovery.map(f => f.shortName.split(' ')[0]).join(' and ');
    const pppDatesArr = recovery.map(f => new Date(f.pppStartDate).toLocaleDateString('en-US', { month: 'short' }));
    const uniqueDates = pppDatesArr.filter((v, i, a) => a.indexOf(v) === i);
    const pppDates = uniqueDates.join(' and ');
