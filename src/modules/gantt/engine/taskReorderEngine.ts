...
export function getTaskParentId(task: any): number {
  return (
    Number(task?.parent ?? task?.parentTaskId ?? task?.parent_task_id ?? 0) || 0
  );
}
...
function buildSiblingGroups(tasks: any[]) {
  const position = new Map(
    tasks.map((task: any, index: number) => [
      task.id,
      getTaskSortOrder(task, index),
    ])
  );
  const originalIndex = new Map(
    tasks.map((task: any, index: number) => [task.id, index])
  );
...
function flattenSiblingGroups<T extends { id: number }>(
  tasks: T[],
  childrenByParent: Map<number, any[]>,
  position: Map<number, number>,
  originalIndex: Map<number, number>,
  rootParentIds: number[]
): T[] {
  const ordered: T[] = [];
  const seen = new Set<number>();

  const walk = (parentId: number) => {
    for (const task of (childrenByParent.get(parentId) || []) as T[]) {
  ...
  for (const task of [...tasks].sort(
    compareTaskOrder(position, originalIndex)
  )) {
    if (!seen.has(task.id)) {
      seen.add(task.id);
      ordered.push(task);
...
export function sortTasksForHierarchyDisplay<T extends { id: number }>(
  tasks: T[]
): T[] {
  const { childrenByParent, position, originalIndex, rootParentIds } =
    buildSiblingGroups(tasks);
  return flattenSiblingGroups(
    tasks,
    childrenByParent,
    position,
    originalIndex,
...
export function getSiblingOrderDebug(
  tasks: any[],
  ...
  return (childrenByParent.get(getTaskParentId(selected)) || []).map(
    (task: any) => ({
      id: task.id,
      text: task.text ?? task.taskName ?? `Task ${task.id}`,
      parent: getTaskParentId(task),
      sort_order: getTaskSortOrder(task, 0),
    })
  );
}

export function getSiblingOrderState(
  tasks: any[],
  selectedTaskId: number | null
) {
  if (!selectedTaskId)
    return { index: -1, count: 0, parentId: 0, task: null as any | null };
  const selected =
    tasks.find((task: any) => task.id === selectedTaskId) || null;
  if (!selected)
    return { index: -1, count: 0, parentId: 0, task: null as any | null };
  ...
  const siblings = childrenByParent.get(parentId) || [];
  return {
    index: siblings.findIndex((task: any) => task.id === selectedTaskId),
...
export function buildManualHierarchyOrder(
  tasks: any[],
  selectedTaskId: number,
  direction: TaskReorderDirection
): TaskReorderUpdate[] | null {
  const selected = tasks.find((task: any) => task.id === selectedTaskId);
  ...
  if (selectedIndex < 0 || swapIndex < 0 || swapIndex >= siblings.length)
    return null;
  ...
  [siblings[selectedIndex], siblings[swapIndex]] = [
    siblings[swapIndex],
    siblings[selectedIndex],
  ];
