...
describe("0031 Projects without PPP migration (content + journal)", () => {
  ...
  it("registers the newest migration as the final journal entry after 0038", () => {
    const entries = journal.entries;
    const last = entries[entries.length - 1];
    expect(last.tag).toBe("0040_mw_inspection_audit");
    expect(last.idx).toBe(entries.length - 1);
    expect(last.when).toBeGreaterThan(entry!.when);
    const previous = entries[entries.length - 2];
    expect(previous.tag).toBe("0039_decommission_maintenance_planning");
    expect(entries[entries.length - 3].tag).toBe("0038_monthly_kpi_situation");
  });
